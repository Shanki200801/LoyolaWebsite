...
export default function Gallery() {
  ...
  return (
    <div className="container m-auto px-16 py-8 bg-slate-300">
      <div className="text-center text-2xl font-bold text-indigo-900 m-4 ">GALLERY</div>
      <div className="tabs flex items-center justify-center">
        <button onClick={() => setPhotoIndex('highlights')} className={`tab text-lg tab-lifted tab-lg tab-rounded-lg w-1/5   ${photoIndex === 'highlights' ? 'bg-navbar-theme text-white ': 'bg-slate-400 text-black' } m-2 rounded-xl `}>
          Highlights
        </button>
        <button onClick={() => setPhotoIndex('photos')} className={`tab text-lg tab-lifted tab-lg tab-rounded-lg w-1/5 m-2 rounded-xl ${photoIndex === 'photos' ?  'bg-navbar-theme text-white': 'bg-slate-400 text-black' }`}>Photos</button>
      </div>
